...
public class App {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < productAmount; i++) {
            System.out.println("Введите название и цену продукта №" + (i+1) + " (формат: Название=Цена): ");
            String[] productDB = scanner.nextLine().trim().split("=");
            if (productDB.length != 2) {
                System.out.println("Ошибка! Формат: Название=Цена (например: Торт=1000)");
                i--;
                continue;
            } else {
                int priceProduct = Integer.parseInt(productDB[1].trim());
                Product product = new Product(productDB[0].trim(), priceProduct);
